...
\title{Proposal for True Random Generation on the APQ8060 Dragonboard}
...
a RTNG in a power friendly method. There are many existing TRNG implementations both in software and hardware. The target platform of this paper,
...
In the Linux kernel, including the Android kernel, there is an entropy pool. This pool is populated via environmental noise from device drivers, timing 
differences between interrupts, and other various sources. These sources are treated as true random number generation and therefore create entropy suitable 
for key generation and other
