...
void do_test(const char *fn)
{
  ...
  //Rob: whoever wrote this code should be shot.  We're just going to assume
  //that we want to run all tests...
  bday(fn);
  operm5(fn);
...
void printHelp()
{
  ...
  puts("\tCongratulations, Rob says you're running all of them.\n");
  puts("\tUsage: diehard -i <input file>\n");
...
{
  //Rob: get filename from command-line.  None of this read in filename garbage.
  const char* fn = "infile.dat";
  int arg = 0;
  ...
  //diehard();
  //Rob: skip printing all that nonsense, just jump to the tests
  do_test(fn);
